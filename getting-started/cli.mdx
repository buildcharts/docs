...
| ---------- | ----------------------------------------------------------------------------------------- |
| `generate` | Validates metadata, pulls charts, runs plugins, and writes `.buildcharts/docker-bake.hcl` |
| `init`     | Scaffolds `build.yml` and `charts/buildcharts/Chart.yaml` for a .NET repository          |
| `pull`     | Downloads a single chart by tag or digest                                                |
| `summary`  | Exports a summary from the latest Docker Buildx history                                  |
| `update`   | Resolves OCI chart dependencies and refreshes `charts/buildcharts/Chart.lock`            |
| `version`  | Prints tool and runtime details                                                          |
...
<ResponseField name="reference" type="argument">
...
Prints runtime version details.
