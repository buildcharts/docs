...
<Tip>
  In [.NET 9](https://learn.microsoft.com/en-us/dotnet/core/whats-new/dotnet-9/sdk#run-tests-in-parallel), tests for the same project run in parallel across different target frameworks.
</Tip>
...
If you use a lock file, the digest is available in `charts/buildcharts/Chart.lock`. That makes the lock file the natural source for verification input during CI or review.
