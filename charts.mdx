...
If the lock file exists, `buildcharts generate` uses it to detect dependency drift before it renders the Bake plan. If it does not exist, generation still works, but chart resolution follows the current registry tags instead of pinned digests.
